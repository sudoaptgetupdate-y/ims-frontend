        ...
        const fetchData = async () => {
            ...
            try {
                const [assetRes, historyRes] = await Promise.all([
                    axios.get(`http://localhost:5001/api/inventory-items/${assetId}`, { headers: { Authorization: `Bearer ${token}` } }),
                    axios.get(`http://localhost:5001/api/inventory-items/${assetId}/history`, { headers: { Authorization: `Bearer ${token}` } })
                ]);
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                            {history.length > 0 ? history.map(h => (
                                ...
                                <tr><td colSpan="3" className="p-4 text-center text-muted-foreground">No assignment history found for this asset.</td></tr>
                            )}
                        </tbody>
                    </table>
                </CardContent>
