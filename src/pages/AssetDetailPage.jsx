...
export default function AssetDetailPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [assetRes, historyRes, summaryRes] = await Promise.all([
                axios.get(`http://localhost:5001/api/assets/${assetId}`, { headers: { Authorization: `Bearer ${token}` } }),
                axios.get(`http://localhost:5001/api/assets/${assetId}/history`, { headers: { Authorization: `Bearer ${token}` } }),
                axios.get(`http://localhost:5001/api/assets/${assetId}/summary`, { headers: { Authorization: `Bearer ${token}` } })
            ]);
    ...
    const handleAction = async (actionType) => {
        try {
            await axios.patch(`http://localhost:5001/api/assets/${assetId}/${actionType}`, {}, {
